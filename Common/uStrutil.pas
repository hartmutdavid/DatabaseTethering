...
uses
  ...
  Winapi.ShellAPI, IdHashMessageDigest, WinSock, TlHelp32;

type
   ctWgKeysSpez = record
    iPos: Integer;    { Position des Einzelkeys }
    iLng: Integer;    { Laenge des Einzelkeys }
  end;
...
function GetGlsParts(sFullGls: String; var sHBz, sUBz, sGlsNr: String): Boolean;
   { Liefert zu einer vollstaendigen Gleisangabe die einzelnen Teile }
...
begin
 l_iSize := 1024;
 SetLength(Result, l_iSize);
 if GetUserName(PChar(Result), l_iSize) then
   SetLength(Result, l_iSize-1)
 else
   RaiseLastOSError;
end;
...
function FormatDateTimeSQLPart(dt: TDateTime): String;
begin
 if uGlobal.g_lIsMSSQL then
   Result := FormatDateTime(FullDateTimeFormat,dt)
 else if uGlobal.g_lIsSQLite then
...
begin
 if uGlobal.g_lIsMSSQL then
   Result := 'CONVERT(DATETIME,''' + FormatDateTime(FullDateTimeFormat,dt) + ''',104)'
 else if uGlobal.g_lIsSQLite then
...
begin
 if uGlobal.g_lIsMSSQL then
   Result := 'CONVERT(DATETIME,''' + FormatDateTime(FullDateTimeFormat,dt) + ''',101)'
 else if uGlobal.g_lIsSQLite then
