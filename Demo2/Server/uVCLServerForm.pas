...
begin
 ...
 if Assigned(l_oClientInfo) then begin
   ...
   if AResource.ResType = TRemoteResourceType.Data then begin
     ...
     txaProt.Lines.Add('- Resource is a String: ' + l_sSendStr);
       ...
       if Length(l_oClientInfo.SendList) > 0 then begin
         ...
         if l_oClientInfo.ActIdxOfSendList >= Length(l_oClientInfo.SendList) then
           ...
           if (l_oSendDesc.SType = TEnumSendType.MSG_TBL_NEW)  or
             ...
             //
             txaProt.Lines.Add('-> Next Server Action - Send Stream: ' + l_oSendDesc.ToString);
             ...
             // Text verschicken ...
             txaProt.Lines.Add('-> Next Server Action - Send String: ' + l_oSendDesc.ToString);
...
begin
 ...
 if Database.Connected then begin
   ...
   if tblSrc.Active then
     tblSrc.Close;
   tblSrc.TableName  := Database.EncodeObjectName('', '', '',i_oSendDesc.SText);
   //
        ...
        if l_lOk and l_lDoTrans then begin
          ...
          try
            try
              ...
              txaProt.Lines.Add('- Memory Table Stream-Size: ' + IntToStr(l_oMemTableStream.Size));
              txaProt.Lines.Add('- Compressed Stream-Size: ' + IntToStr(l_oMemComprStream.Size));
