...
begin
 ...
 if m_oServerInfo.ServerId = l_sServerId then begin
   if aResource.ResType = TRemoteResourceType.Data then begin
     ...
     txaProt.Lines.Add(' as Stream - at: ' + DateTimeToStr(Now) + '; Size: ' + IntToStr(AResource.Value.AsStream.Size));
...
begin
 ...
 if Database.Connected then begin
   ...
   //
   l_sTableName := Database.EncodeObjectName('', '', '',i_oSendDesc.SText);
 ...
 if Assigned(l_oMemTable) then begin
   if (i_oSendDesc.SType = TEnumSendType.MSG_TBL_NEW) then begin
     try
